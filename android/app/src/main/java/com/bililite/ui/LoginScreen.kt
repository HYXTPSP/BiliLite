...
fun LoginScreen(vm: LoginViewModel, onDone: () -> Unit) {
    ...
    Column(
        Modifier.fillMaxSize().background(Color(0xFFFAFAFA)).padding(24.dp),
        horizontalAlignment = Alignment.CenterHorizontally
    ) {
        Spacer(Modifier.height(24.dp))
        Text("BiliLite", fontSize = 24.sp, color = Color(0xFF1C1C1E))
        Spacer(Modifier.height(8.dp))
        Text("学习专注型 B 站客户端", fontSize = 13.sp, color = Color(0xFF8E8E93))
        Spacer(Modifier.height(24.dp))
        ...
        TabRow(selectedTabIndex = tab, containerColor = Color.Transparent,
            ...
            listOf("扫码登录", "验证码登录", "密码登录").forEachIndexed { i, t ->
                Tab(selected = tab == i, onClick = { tab = i }, text = {
                    Text(t, color = if (tab == i) Color(0xFF1C1C1E) else Color(0xFF8E8E93), fontSize = 14.sp)
                })
            }
        }
        Spacer(Modifier.height(8.dp))
...
private fun QrTab(vm: LoginViewModel) {
    ...
    Column(horizontalAlignment = Alignment.CenterHorizontally) {
        Spacer(Modifier.height(12.dp))
        when (state) {
            is LoginState.Loading -> CircularProgressIndicator(color = Color(0xFF1C1C1E))
            is LoginState.NeedScan -> {
                ...
                Text("用 B 站 App 扫码登录", fontSize = 14.sp, color = Color(0xFF1C1C1E))
                Text("扫码后在手机确认,等待自动跳转", fontSize = 11.sp, color = Color(0xFF8E8E93))
                Spacer(Modifier.height(14.dp))
                Button(colors = ButtonDefaults.buttonColors(containerColor = Color(0xFF1C1C1E)),
                    onClick = { vm.startPoll() }) { Text("我已扫码,刷新状态", color = Color.White) }
            }
            is LoginState.Scanned -> {
                Text("✓", fontSize = 34.sp, color = Color(0xFF1C1C1E))
                Spacer(Modifier.height(10.dp))
                Text(state.msg, fontSize = 14.sp, color = Color(0xFF1C1C1E))
                Spacer(Modifier.height(12.dp))
                CircularProgressIndicator(color = Color(0xFF1C1C1E), strokeWidth = 3.dp)
            }
            is LoginState.Success -> { }
            is LoginState.Expired -> {
                Text("二维码已失效", color = Color(0xFF8E8E93))
                Spacer(Modifier.height(14.dp))
                Button(onClick = { vm.generate() }, colors = ButtonDefaults.buttonColors(containerColor = Color(0xFF1C1C1E))) { Text("重新获取", color = Color.White) }
            }
    ...
    Column(horizontalAlignment = Alignment.CenterHorizontally) {
        ...
        Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
            ...
            OutlinedButton(onClick = {
                ...
                Text(if (countdown > 0) "${countdown}s" else "获取验证码", color = Color(0xFF1C1C1E))
            }
        }
        if (vm.smsMsg.isNotEmpty()) {
            Spacer(Modifier.height(6.dp))
            Text(vm.smsMsg, color = Color(0xFF8E8E93), fontSize = 12.sp)
        }
        Spacer(Modifier.height(16.dp))
